...
use elf_runtime_model_kernel::ArcDayOfWeekParameter;
use std::ops::Deref;
use std::sync::Arc;

pub struct CompiledDayOfWeekParameter {
    parameter: CompiledParameter,
}

impl CompiledDayOfWeekParameter {
    pub fn compile(param: &Arc<ArcDayOfWeekParameter>, tenant_id: &Arc<TenantId>) -> StdR<Self> {
        Ok(CompiledDayOfWeekParameter {
            ...
            ArcTopicDataValue::Str(str) => {
                if str.is_empty() {
                    ...
                    return PipelineKernelErrorCode::ComputeParameterNotADate.msg(format!(
                        "Argument value of day-of-week must be a date or datetime, current is [{}].",
                        str
                    ));
                }
            }
