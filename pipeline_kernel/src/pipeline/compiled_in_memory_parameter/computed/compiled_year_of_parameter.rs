...
use elf_runtime_model_kernel::ArcYearOfParameter;
use std::ops::Deref;
use std::sync::Arc;

pub struct CompiledYearOfParameter {
    parameter: CompiledParameter,
}

impl CompiledYearOfParameter {
    pub fn compile(param: &Arc<ArcYearOfParameter>, tenant_id: &Arc<TenantId>) -> StdR<Self> {
        Ok(CompiledYearOfParameter {
...
impl CompiledYearOfParameter {
    pub fn value_from(&self, in_memory_data: &mut InMemoryData) -> StdR<Arc<ArcTopicDataValue>> {
        let result = match self.parameter.value_from(in_memory_data)?.deref() {
            ...
            ArcTopicDataValue::Date(date) => BigDecimal::from(date.year()),
            ArcTopicDataValue::DateTime(datetime) => BigDecimal::from(datetime.year()),
            ArcTopicDataValue::Str(str) => {
                if str.is_empty() {
                    return Ok(Arc::new(ArcTopicDataValue::None));
                } else if let Ok(date) = str.to_date_loose() {
                    BigDecimal::from(date.year())
                } else {
                    return PipelineKernelErrorCode::ComputeParameterNotADate.msg(format!(
                        "Argument value of year-of must be a date or datetime, current is [{}].",
                        str
                    ));
                }
            }
