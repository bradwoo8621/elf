...
struct DataPathFuncParserDelegate<'a> {
    parser: &'a mut DataPathFuncParser,
    param_start_char_index: usize,
}

impl DataPathAnyFuncParser for DataPathFuncParserDelegate<'_> {
...
impl DataPathFuncParser {
    ...
    fn parse_param(&mut self, param_start_char_index: usize) -> VoidR {
        let mut path_parser = DataPathParser {
        ...
        let delegate = &mut DataPathFuncParserDelegate {
            parser: self,
